/*
 * 本头文件以宏定义的方式声明了动态内存池管理器管理的所有内存池及它们的各个属性值，具体定义在memp.c中。
 * 内存池分三大类:(“可选”是指内存池的有无取决于内核配置文件；“必有”是指内核固有的)
 * 1)(可选)MEMPOOL类型：标准类型的内存池，指由管理器管理的内核各个功能模块控制结构相关的内存池，各种控制结构
 *                池的有无以及池中元素的个数依赖于内核配置文件opt.h中的设置；
 * 2)(可选)MALLOC_MEMPOOL类型：用于动态内存堆管理器的内存池，仅在动态内存堆管理器使用基于固定内存块的内存池策
 *                略并且在用户自定义头文件lwippools.h中声明了自定义内存池时才会存在，并被动态内存池管理
 *                管理；
 * 3)(必有)PBUF_MEMPOOL类型：pbuf结构相关内存池，有两种：一种是pbuf头内存池(PBUF类型)，另一种是完整pbuf内存池
 *                (PBUF_POOL类型)。
